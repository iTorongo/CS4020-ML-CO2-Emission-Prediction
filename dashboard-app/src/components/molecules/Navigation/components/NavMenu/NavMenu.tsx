...
import { PUBLIC_LIST } from "@app/routes/routes.config";
...
const NavMenu = ({ isSidebar, mode }: NavMenuProps) => {
  ...
  const navLinks: RouteItemDef[] = PUBLIC_LIST.filter(
    ...
    >
      {navLinks.map(navItem =>
        navItem.nestedRoutes?.length ? (
          ...
          >
            {navItem.nestedRoutes.map((subItem: RouteItemDef | RouteGroupDef) =>
              "groupTitle" in subItem ? (
                ...
                <Menu.Item
                  key={
                    Array.isArray(subItem.path) ? subItem.path[0] : subItem.path
                  }
                >
